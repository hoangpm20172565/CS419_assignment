...
from svmKernels import myCosineSimilarityKernel
...
C = 100.0  # value of C for the SVMs

# create an instance of SVM with the custom kernel and train it
myModel = svm.SVC(C = C, kernel=myCosineSimilarityKernel)
myModel.fit(X, Y)
...
plt.title("SVM with My Custom Cosine SImilarity Kernel (C = "+str(C)+")")
plt.axis('tight')
